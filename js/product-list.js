        ...
        productListDomString += `
          <div class="card col-12 col-md-6 col-lg-4 wew bounceInLeft">
            <div class="card product">
              <img class="card-img-top" src="imgshop/${product.image}" alt="${product.title}">
              <div class="card-body">
                <h4 class="card-title">${product.title}</h4>
                <p class="card-text">${product.description}</p>
                <button class="btn btn-info" data-toggle="modal" data-target="#productInfoModal" data-id="${product.id}">Подробнее</button>
                <button class="btn btn-primary buy" data-id="${product.id}">₴${product.price} - Купить</button>
              </div>
            </div>
          </div>`;
      });
